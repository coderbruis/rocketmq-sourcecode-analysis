...
/**
 * 1. CommitLog文件以消费偏移量来命名文件名，这样做的好处是只要给出任意的一个消息偏移量，可以通过二分法进行查找，快速定位到CommitLog文件之后，然后用消息偏移量减去当前文件的名称（偏移量），差值就是
 * 消息在该文件的绝对路径了。
 *
 *
...
public class CommitLog {
    ...
    // 可以视作MappedFileQueue为${ROCKET_HOME}/store/commitlog 文件夹对象，而MappedFile则对应commitlog文件夹下的文件对象
    ...
    public SelectMappedBufferResult getData(final long offset, final boolean returnFirstOnNotFound) {
        ...
        // 获取offset所在的commitLog文件
        MappedFile mappedFile = this.mappedFileQueue.findMappedFileByOffset(offset, returnFirstOnNotFound);
        if (mappedFile != null) {
            // 对commitLog大小进行取余，获取到offset的下标位置
     ...
     *
     * 这里byteBuffer传进来的是CommitLog最新存储的消息内容，这里要做什么呢？
     * 答：将byteBuffer中的二进制消息内容解析出来存在DispatchRequest中
     *
     * 值得注意的是，DispatchRequest里没有消息body内容，主要是用于另外存到ConsumeQueue和Index中，不用再存消息body
     *
        ...
        try {
            ...
            if (bodyLen > 0) {
                if (readBody) {
                    ...
                    if (checkCRC) {
                        // 通过crc32算法解压消息体内容，内容存在bytesContent二进制数组里
    ...
    /**
     * 将消息写入CommitLog
     * CommitLog的核心三步骤：1. 获取最新MappedFile对象；2. 往缓冲区写消息；3. 提交刷盘请求；
     *
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            // 1. 获取最新的MappedFile对象(MappedFile对象是：CommitLog、IndexFile、Consumequeue的映射对象)
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        MappedFile unlockMappedFile = null;
        // 从commitlog文件夹下获取最新的commitlog文件
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();

        putMessageLock.lock(); //spin or ReentrantLock ,depending on store config       加锁串行化执行
        try {
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
            this.beginTimeInLock = beginLockTimestamp;
            ...
            // 如果mappedFile为空则表示 /commitLog目录下不存在任何文件，说明本次消息是第一次消息发送，用偏移量0创建第一个commit文件，文件名为：000000000000000000000000
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            if (null == mappedFile) {
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
            if (messageExt.isWaitStoreMsgOK()) {
                // result.getWroteOffset(): 起始刷盘位点      result.getWroteBytes(): 要刷盘消息的字节数
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes());
                service.putRequest(request);
    ...
    public long getMinOffset() {
        ...
        if (mappedFile != null) {
            ...
            if (mappedFile.isAvailable()) {
                // 获取文件名的偏移量，由于文件名就是该commitLog的其实地址，所以此处获取的就是文件的地址偏移量
    ...
    public SelectMappedBufferResult getMessage(final long offset, final int size) {
        // commitlog的文件大小
        int mappedFileSize = this.defaultMessageStore.getMessageStoreConfig().getMappedFileSizeCommitLog();
        // 根据offset查找到所在的commitlog文件
        MappedFile mappedFile = this.mappedFileQueue.findMappedFileByOffset(offset, offset == 0);
        if (mappedFile != null) {
            // 查找到offset在commitlog中的索引位置
            int pos = (int) (offset % mappedFileSize);
    ...
    /**
     * CommitLog刷盘抽象类，数据的刷盘都是从Page Cache中刷到磁盘
     */
     ...
     * 采取堆外内存的异步刷盘
     *
     * CommitRealTimeService 先将消息由堆外内存 -> PageCache -> FlushRealTimeService -> Disk
     *
     */
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 如果距上次提交间隔超过commitDataThoroughInterval，则本次提交忽略commitDataLeastPages，即设置为0
                ...
                try {
                    //把消息提交到内存缓冲区，最终调用的是 MappedFile::commit0 方法，只有达到最少提交页数才能提交成功，否则还在堆外内存中
                    boolean result = CommitLog.this.mappedFileQueue.commit(commitDataLeastPages);
                    long end = System.currentTimeMillis();
                    // TODO 看commit逻辑，false会一直为false，所以会一直执行flushCommitLogService#wakeup()
                    if (!result) {
                        // 结果为false并不代表提交失败，而是表示有数据提交成功了，唤醒刷盘线程执行刷盘操作。
                    ...
                    // 通过CountDownLatch2来阻塞当前线程，必须等待commit完成后才继续执行后面逻辑
                    // 阻塞200ms
    ...
    class GroupCommitService extends FlushCommitLogService {
        // 1. 通过requestsWrite和requestsRead来实现读写分离，好处就是对于同步刷盘请求可以不用加锁，提高并发度
        // 2. requestsWrite相当于同步刷盘任务暂存器，requestsRead相当于GroupCommitService这个线程要处理任务的处理容器，任务的存储与处理容器分开，避免了提交时的锁冲突
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        DefaultAppendMessageCallback(final int size) {
            ...
            // 消息存储内存对象，size是消息最大存储大小，END_FILE_MIN_BLANK_LENGTH是8字节大小，表示消息存储对象mappedFile最少空闲8字节，高4字节存储当前文件的剩余空间，低4字节存储魔数CommitLog.BLANK_MAGIC_CODE
        ...
        /**
         * 往mappedFile中追加消息，顺序写
         *
         * 此处只是将消息追加到内存中，需要根据采取的是同步刷盘方式还是异步刷盘方式，将内存中的数据持久化到磁盘中。
         *
         * @param fileFromOffset        当前mappedFile的文件起始偏移量
         * @param byteBuffer            和原ByteBuffer共享的内存区域（原mappedFile文件）
         * @param maxBlank              当前mappedFile还可以写的字节数大小，也就是空闲空间
         * @param msgInner              要往mappedFile存储的消息
         * @return
         */
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 计算消息长度(偏移量)，包括：sysFlag + body + topic + properties
            // 获取该消息在消息队列的物理偏移量。CommitLog中保存了当前所有消息队列的当前待写入偏移量
            ...
            // 消息长度 + END_FILE_MIN_BLANK_LENGTH 大于空闲空间，会重新创建一个新的CommitLog文件来存储消息
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
                ...
                // 将消息存储在了MappedFile对应的内存映射ByteBuffer中，并没有刷新到磁盘
                // 【核心】!!! 这里就是将消息存储到了mappedFile里，因为byteBuffer是映射的内存地址，但是这边只是将消息写入缓冲区，还未实际刷盘
